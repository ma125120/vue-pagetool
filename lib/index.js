...
var createPage = function(page,root_dir,prev_dir, prev_name = '') {
  ...
  if(!page.pages) {
    //新建最底层的vue
    fs.touch(`${prev_dir}/${firstUpper(prev_name)}${firstUpper(page.name)}.vue`,getPageComponentTemp(page,root_dir,prev_name));
    //新建各页面的组件
    page.components && createComponent(page,root_dir,prev_name);
    //新建scss文件
    fs.touch(`${root_dir}/../static/css/${prev_name}/${page.name}.scss`,'');
  } else {
